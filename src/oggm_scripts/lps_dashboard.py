...
def initialise_dashboard_model(rgi_ids:list=None):
    ...
    if rgi_ids:
        gdir = datacube_calib.get_gdir_for_calibration(gdirs, "rgiid", rgi_ids[0])
    else:
...
def run_calibration(model_matrix, gdir, ref_mb):
    ...
    for matrix_name, model_params in model_matrix.items():
        ...
        mb_geodetic = (
            ref_mb.loc[ref_mb.period == geo_period].loc[gdir.rgi_id].dmdtda * 1000
        )
...
def set_calibration(gdir, datacube):
    ...
    model_matrix = {
        "DailySfc_Cryosat": {
            ...
            "geo_period": "2010-12-30_2020-01-12",
            "daily": True,
            "extra_kwargs": sfc_model_kwargs,
        },
    }

    mb_model_calib, mb_model_flowlines, smb_daily =  run_calibration(model_matrix=model_matrix, gdir=gdir, ref_mb=ref_mb)
    return mb_model_calib, mb_model_flowlines, smb_daily

def get_data():
    gdir = initialise_dashboard_model()
...
def plot_data(smb:dict, gdir, datacube,ref_year:int=2017, resample:bool=False):
    ...
    smb=smb, ref_year=ref_year, glacier_name=gdir.name, datacube=datacube, gdir=gdir, resample=resample
    )
    return layout
